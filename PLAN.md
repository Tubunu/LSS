...
- 当前阶段：Phase 5 — Processing UX（READY）。
...
- Phase 4 结果：PASS。实现多帧稳定边缘块分析，检测顶部栏、底部栏和内部 / 悬浮固定区域，生成 2D exclusion mask 参与 overlap、texture 与 seam 评分；seam 增加边缘切割惩罚，小幅回滚帧可跳过并继续向下拼接。为避免周期性正文误判，只有同时存在尺寸合理的顶部与底部锚点时才启用内部候选；25 个真机测试全部通过，包含 Phase 3 全量回归。
