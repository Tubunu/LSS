...
Phase 0 至 Phase 4 已在 iPhone 16 Pro / iOS 27.0 真机通过：
...
- 高级拼接可检测顶部、底部和悬浮固定区域并生成 exclusion mask；
- 接缝会避开固定 UI 与高边缘区域，小幅回滚帧可安全跳过后继续拼接。
...
当前仓库已完成 Phase 4，正进入捕获后的 Processing UX。编辑、导入、历史和导出将按 `PLAN.md` 后续 Phase 顺序实现。
